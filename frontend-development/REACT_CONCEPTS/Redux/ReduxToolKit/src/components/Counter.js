...
const Counter = () => {
  ...
  const counter = useSelector((state) => state.counter.counter); //accessing counter property of state from the store.
  const showCounter = useSelector((state) => state.counter.showCounter); //accessing showCounter property of the state from the store.

  function incrementCounter() {
    dispatch(counterAction.increment()); //dispatch {type: some_action_identifier}
  }
  function increaseHandler() {
    dispatch(counterAction.increase(5)); //dispatch {type: some_action_identifier, payload: 5}
  }
