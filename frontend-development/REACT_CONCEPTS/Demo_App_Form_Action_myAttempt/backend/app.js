...
app.use(express.json());//enables json parsing. it ensures that req.body json format data is converted into object for use in the server.

app.get('/opinions', async (req, res) => {
  try {
    const opinions = await loadOpinions();
    res.json(opinions);//sends array in json format
  } catch (error) {
...
app.post('/opinions', async (req, res) => {
  ...
  //make sure u wait 1 second before the next line of code is executed after me is executed
