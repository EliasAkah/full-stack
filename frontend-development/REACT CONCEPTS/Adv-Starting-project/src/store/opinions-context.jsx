...
export function OpinionsContextProvider({ children }) {
  ...
  async function upvoteOpinion(id) {
    ...
    if (!response.ok) {
      return; // prevents the state or UI from updating if the data fails to get to the backend
    }
  ...
  async function downvoteOpinion(id) {
    ...
    //updates state or UI if reponse is successful and the backend is updated
    setOpinions((prevOpinions) => {
      return prevOpinions.map((opinion) => {
        if (opinion.id === id) {
          return { ...opinion, votes: opinion.votes - 1 };
